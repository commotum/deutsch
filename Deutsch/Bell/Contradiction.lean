...
/-- A local model reproduces the quantum agreement probability at every pair of the
...
/-- Reproduction of the quantum family gives the three quarter-agreement premises used
...
/-- Reproducing the quantum family also gives probability-one agreement at every equal-setting
pair. -/
...
/--
There is no normalized nonnegative, setting-independent
distribution over deterministic local response tables that both has perfect equal-setting
agreement on positive-weight support and reproduces the EPR agreement probabilities at
the three named settings.
-/
theorem epr_three_settings_refute_local_assignments
    (weight : LocalAssignment → ℝ)
...
/-- Negated packaging of the contradiction, convenient for downstream reuse. -/
theorem no_local_assignments_reproduce_epr_three_settings
    (weight : LocalAssignment → ℝ)
    (weight_nonnegative : ∀ assignment, 0 ≤ weight assignment)
    (weight_normalized : ∑ assignment, weight assignment = 1)
    (perfect_support : HasPerfectEqualSettingSupport weight) :
  ...
  intro reproduces
  exact epr_three_settings_refute_local_assignments
    weight weight_nonnegative weight_normalized perfect_support reproduces

/--
Observable-probability form of the contradiction. Perfect equal-setting support is no
...
-/
theorem epr_three_settings_refute_normalized_local_model
    (weight : LocalAssignment → ℝ)
    (weight_nonnegative : ∀ assignment, 0 ≤ weight assignment)
    (weight_normalized : ∑ assignment, weight assignment = 1)
    (reproduces : ReproducesThreeSettingQuantumAgreements weight) :
...
three-setting agreement table. -/
theorem no_normalized_local_model_reproduces_epr_three_settings
    (weight : LocalAssignment → ℝ)
    (weight_nonnegative : ∀ assignment, 0 ≤ weight assignment)
    (weight_normalized : ∑ assignment, weight assignment = 1) :
    ¬ ReproducesThreeSettingQuantumAgreements weight := by
